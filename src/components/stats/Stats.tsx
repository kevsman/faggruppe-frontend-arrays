...
const StatsComponent: React.FC<Props> = ({ players }) => {
    ...
    const getSortedPlayers = () => {
         ...
         */
        return filteredPlayers.sort((first, second) => isSortedAsc ? first.cash - second.cash : second.cash - first.cash)
    }
    ...
    const addTopPlayer = () => {
         ...
         * TODO: Husk å legg inn eksempler på reducer med {} as TYPE
         */
        ...
        // const topPlayer = players.reduce((maxPlayer, player) => maxPlayer.cash > player.cash ? maxPlayer : player, {} as Player)
    ...
    const getCashSum = () => {
        ...
        // players.forEach(player => sum += player.cash);
        // return players.reduce((sum, player) => sum += player.cash, 0);
    ...
    useEffect(() => {
         ...
         * Oppgave 5:
         * - Søk etter spillere
         * - Filtrer på spillernavn (player.name)
         */

        const filteredPlayers = players.filter(player => player.name.toLowerCase().includes(searchKey));
