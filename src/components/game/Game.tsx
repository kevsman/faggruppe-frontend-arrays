...
const GameComponent: React.FC<Props> = ({ players }) => {
    ...
    const newGame = () => {
        ...
        setPlayerRanks(playersInGame, table); // Oppgave 7 finner du her
    ...
    return (
        <>
            <h4>Spill</h4>
            <div>
                {isGameDone() && <div>Vinner {GetWinners()}</div>}
                <div>
                    {!isGameDone() && <button onClick={() => addCardToTable()}>Trekk neste</button>}
                    {isGameDone() && <button onClick={() => newGame()}>Nytt spill</button>}
                </div>
                <PlayerOuterWrapper>
